...
let replyToMsgId = null, replyToText = "";
...
function initMesh() {
    try {
        ...
        myPeerInstance.on('connection', conn => { handleIncomingRequest(conn); setupConn(conn); });
...
function sendMessage() {
    ...
    for (const w of bannedWords) {
        if (txt.toLowerCase().includes(w)) { showToast("Message blocked by safety filter"); inp.value = ""; return; }
    }
    sendBundle("text", txt);
    inp.value = "";
    cancelReply();
...
function sendBundle(contentType, payload) {
    ...
    const msg = {
        ...
        time, replyTo: replyToMsgId, replyText
    };
    ...
    broadcastToMesh({
        ...
        replyTo: replyToMsgId, replyText
    });
...
function blockCurrentPeer() { closeAllMenus(); if(!currentChatPeer||!confirm("Block "+currentChatPeer+"?")) return; activeConnections=activeConnections.filter(c=>c.peer!==currentChatPeer); delete chatData[currentChatPeer]; goBackToList(); showToast(currentChatPeer+" blocked"); }
...
function listenCallStream(callObj, type) {
    callObj.on('stream', remoteStream => {
        ...
        document.getElementById("video-grid").classList.toggle("hidden", type !== 'video');
        startCallTimer();
    });
...
function spawnNearbyNodes(lat, lng, mapInst) {
    ...
    nodes.forEach(n => {
        L.marker([lat+n.lo, lng+n.ln]).addTo(mapInst).bindPopup(
            `<b>${n.id}</b><br>Online<br><button class="map-connect-btn" onclick="connectToPeer('${n.id}')">Connect</button>`
        );
        updateOnlineUsers(n.id, n.id, true);
    });
}
...
function fixKeyboard() {
    ...
    if (window.visualViewport) {
        window.visualViewport.addEventListener("resize", () => {
            ...
            const kbHeight = fullH - vvh;

            if (kbHeight > 50) {
                inputArea.style.transform = `translateY(-${kbHeight}px)`;
                msgContainer.style.paddingBottom = (kbHeight + 80) + "px";
            } else {
    ...
    document.getElementById("msg-input")?.addEventListener("focus", () => {
        setTimeout(() => {
            document.getElementById("msg-input")?.scrollIntoView({ behavior: "smooth", block: "center" });
    ...
    document.addEventListener("focusout", e => {
        if (e.target.id === "msg-input") {
            setTimeout(() => {
                ...
                if (msgContainer) msgContainer.style.paddingBottom = "80px";
                window.scrollTo(0, 0);
            }, 100);
        }
    });
