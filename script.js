...
// Safe storage wrapper - prevents crash if localStorage is blocked (some WebViews/private mode)
const safeStorage = {
    _memory: {},
    _available: (() => {
    ...
    getItem(key) {
        if (this._available) { try { return window.localStorage.getItem(key); } catch (e) { return this._memory[key] || null; } }
        return this._memory[key] || null;
    },
    setItem(key, val) {
        if (this._available) { try { window.localStorage.setItem(key, val); return; } catch (e) {} }
        this._memory[key] = val;
    },
    removeItem(key) {
        if (this._available) { try { window.localStorage.removeItem(key); return; } catch (e) {} }
...
function checkAppLock() {
    const pin = safeStorage.getItem("gm_pin");
    if (pin) { showEl("lock-screen"); } 
    else {
        const phone = safeStorage.getItem("gm_phone");
...
function checkPin() {
    const saved = safeStorage.getItem("gm_pin");
    if (pinBuffer === saved) {
        hideEl("lock-screen");
        const phone = safeStorage.getItem("gm_phone");
...
function verifyAndLogin() {
    ...
    if (pin.length === 4) safeStorage.setItem("gm_pin", pin);
    safeStorage.setItem("gm_phone", phone);
    executeLogin(phone);
}
...
function logoutApp() {
    ...
    safeStorage.removeItem("gm_phone");
    safeStorage.removeItem("gm_pin");
    location.reload();
}
...
function toggleVoiceRecord() {
    ...
    if (!isRecordingAudio) {
        navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
            ...
            mediaRecorderInstance.ondataavailable = e => { if (e.data && e.data.size > 0) recordedAudioChunks.push(e.data); };
...
// Fixes: input bar jumping/hiding when on-screen keyboard opens (Android/iOS)
if (window.visualViewport) {
    ...
    const fixViewport = () => {
        ...
        if (appShell) appShell.style.height = vv.height + 'px';
    };
    vv.addEventListener('resize', fixViewport);
    vv.addEventListener('scroll', fixViewport);
}

// Prevent pull-to-refresh / overscroll bounce on the whole page
document.addEventListener('touchmove', (e) => {
    if (e.target.closest('#messages-container, #chat-list-container, .sos-content, .profile-content, .modal-box')) return;
    e.preventDefault();
}, { passive: false });

// Re-focus chat input correctly after keyboard closes (Android quirk)
