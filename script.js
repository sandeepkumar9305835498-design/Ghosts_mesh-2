...
function renderCreditsUI() {
    ...
    amountEl.innerText = hasProfile ? getCredits() : "—";

    if (!hasProfile) {
        if (eligMsg) {
            eligMsg.innerText = "Create your Ghost ID first to start earning credits";
            eligMsg.classList.remove("hidden");
        }
        earnBtns.forEach(b => b.disabled = true);
    } else {
        if (eligMsg) eligMsg.classList.add("hidden");
        earnBtns.forEach(b => b.disabled = false);
    }
...
// ----- Spending -----
function openPremiumModal() { closeAllMenus(); showEl("premium-modal"); }
