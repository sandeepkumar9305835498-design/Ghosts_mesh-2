...
// ===== WHATSAPP-STYLE MAIN TABS (Chats / WiFi / SOS) =====
...
function setActiveMainTab(index) {
    ...
    if (index === 2) { initSOSMap(); fetchSOSInfo(); } // SOS tab needs its map/data live
    if (index === 3) { refreshOnlineUsers(); } // Online tab needs a fresh peer list
}
...
function openSOS() { closeAllMenus(); showScreen("chatlist-screen"); scrollToMainTab(2); }
function closeSOS() { scrollToMainTab(0); }
...
function broadcastToMesh(obj) {
    ...
    activeConnections.forEach(c => {
        try {
            ...
            // everyone else — especially for SOS alerts.
            console.error("Send failed to", c?.peer, e);
            dead.push(c);
        }
    });
    // Prune connections that just proved themselves dead so future
    // broadcasts (like the SOS 15s heartbeat) don't keep failing on them.
    if (dead.length) {
...
function fetchSOSInfo() {
    try {
        const peersEl = document.getElementById("sos-peers");
        if (peersEl) peersEl.innerText = activeConnections.filter(c => c?.open).length + " peers";
    } catch(e) { console.error(e); }
    try {
    ...
    navigator.geolocation?.getCurrentPosition(pos => {
        userLat = pos.coords.latitude; userLng = pos.coords.longitude;
        hasRealLocation = true;
        const el = document.getElementById("sos-my-location");
        if (el) el.innerText = userLat.toFixed(4) + ", " + userLng.toFixed(4);
    }, () => { const el = document.getElementById("sos-my-location"); if (el) el.innerText = "Location blocked"; });
}
...
function toggleSOS() {
    ...
    if (!sosActive) {
        sosActive = true;
        // Send the alert FIRST — this must happen no matter what,
        // even if some UI element below fails to update.
        const reached = broadcastSOSNow();
        try {
            if (btn) { btn.innerHTML = "STOP SOS ALERT"; btn.classList.add("active-sos"); }
            const t = document.getElementById("sos-title"); if (t) t.innerText = "SOS ACTIVE!";
            const d = document.getElementById("sos-desc"); if (d) d.innerText = "Broadcasting your location to all nearby Ghost Mesh peers...";
        } catch(e) { console.error("SOS UI update failed", e); }
        sosInterval = setInterval(broadcastSOSNow, 15000);
        if (reached > 0 && hasRealLocation) {
            showToast(`SOS sent to ${reached} connected peer${reached === 1 ? '' : 's'} with your location`);
        } else if (reached > 0 && !hasRealLocation) {
            showToast(`⚠️ SOS sent to ${reached} peer(s), but your GPS location isn't available — enable location for it to help`);
        } else {
            showToast("⚠️ SOS armed, but you have 0 connected peers right now — no one will receive it until someone connects");
        }
        if (navigator.vibrate) navigator.vibrate([300,100,300,100,300]);
    } else {
        sosActive = false; clearInterval(sosInterval);
        try {
            if (btn) { btn.innerHTML = "SEND SOS ALERT"; btn.classList.remove("active-sos"); }
            const t = document.getElementById("sos-title"); if (t) t.innerText = "Emergency Help";
            const d = document.getElementById("sos-desc"); if (d) d.innerText = "Press SOS to alert all nearby Ghost Mesh users with your live GPS location";
        } catch(e) { console.error("SOS UI update failed", e); }
...
function broadcastSOSNow() {
    try {
        const peerCount = activeConnections.filter(c => c?.open).length;
        broadcastToMesh({ type: "sos", sender: userGhostID, senderName: userDisplayName, lat: userLat, lng: userLng, senderDP: userCurrentDP, time: nowTime(), hasLocation: hasRealLocation === true });
        return peerCount;
    } catch(e) {
        console.error("broadcastSOSNow failed", e);
        return 0;
    }
}
...
function showNearbyAlert(data) {
    ...
    if (!list) return;
    const locText = data.hasLocation && typeof data.lat === "number" && typeof data.lng === "number"
    ...
    item.innerHTML = `<b>${data.senderName || data.sender}</b><br>${locText}<br>${data.time}<br><button onclick="connectToPeer('${data.sender}')" style="background:var(--danger);color:white;border:none;padding:5px 14px;border-radius:8px;margin-top:6px;cursor:pointer;font-weight:700;min-height:36px;">Respond</button>`;
    list.prepend(item);
}
